<?php
// Script para inserir as rutas finais no banco de datos
// Execute: php php/update_rotas.php
...
try {
    ...
    // Limpar todas as rotas existentes e inserir novas
    $conn->query("DELETE FROM tabela_precos");
    echo "Rotas anteriores eliminadas.\n";
    
    // Inserir novas rotas
    $stmt = $conn->prepare("INSERT INTO tabela_precos (origem, destino, valor, regiao) VALUES (?, ?, ?, ?)");
    
    $inseridas = 0;
    foreach ($rotas as $rota) {
        $stmt->bind_param("ssds", $rota["origem"], $rota["destino"], $rota["valor"], $rota["regiao"]);
        $stmt->execute();
        $inseridas++;
    }
    
    echo "Total de rutas insertadas: $inseridas\n";
    echo "Rutas actualizadas con éxito!\n";
