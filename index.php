...
if ($html !== false) {
  ...
  if (!empty($logoUrl)) {
    $version = time();
    $logoUrlWithVersion = htmlspecialchars($logoUrl) . '?v=' . $version;
    
    // Replace OpenGraph and Twitter images
    $html = preg_replace('/<meta property=["\']og:image["\'] content=["\'][^"\']*["\']\s*\/?>/i', '<meta property="og:image" content="' . htmlspecialchars($logoUrl) . '">', $html);
    $html = preg_replace('/<meta name=["\']twitter:image["\'] content=["\'][^"\']*["\']\s*\/?>/i', '<meta name="twitter:image" content="' . htmlspecialchars($logoUrl) . '">', $html);
    
    // Replace favicon and apple-touch-icon links (including hashed versions generated by bundlers)
