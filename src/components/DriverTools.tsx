...
import {
  ...
  Camera, Loader2, ZoomIn, ZoomOut, AlertTriangle, Download, Users,
} from 'lucide-react';
          ...
          {taxaCarro6Valor > 0 && (
            <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg">
              <input
                type="checkbox"
                id="carro6Calc"
                checked={carro6Lugares}
                onChange={e => setCarro6Lugares(e.target.checked)}
                className="w-5 h-5 rounded border-border text-accent focus:ring-accent"
              />
              <label htmlFor="carro6Calc" className="text-sm cursor-pointer">
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-col gap-2 max-w-[420px] mx-auto">
        <Button
          className="w-full h-11 rounded-xl gap-2 font-semibold text-base bg-green-600 hover:bg-green-700 text-white"
          onClick={handleShare}
        >
