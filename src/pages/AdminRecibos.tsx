...
import {
  ...
  ArrowDownAZ, ArrowUpAZ, Loader2, Send,
} from 'lucide-react';
...
const AdminRecibos: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return items;
  }, [recibos, search, periodoFilter, sortField, sortDir, motoristaNomeById]);
                ...
                <div className="space-y-1.5">
                  {detalhes.tipo_tarifa && (
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-medium">R$ {(detalhes.valor_base || 0).toFixed(2).replace('.', ',')}</span>
                    </div>
                  )}
                  ...
                  {detalhes.taxa_bagagem != null && (
                    <div className="flex justify-between text-xs">
                      <span className="text-muted-foreground">Feira/Bagagem</span>
                      <span className="font-medium text-orange-400">+R$ {detalhes.taxa_bagagem.toFixed(2).replace('.', ',')}</span>
                    </div>
                  )}
                  {detalhes.carro_6 != null && (
                    <div className="flex justify-between text-xs">
                      <span className="text-muted-foreground">Carro 6 lugares</span>
                      <span className="font-medium text-cyan-400">+R$ {detalhes.carro_6.toFixed(2).replace('.', ',')}</span>
                    </div>
                  )}
