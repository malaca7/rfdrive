  ...
  return (
    <div className="h-[100dvh] w-full bg-background flex flex-col overflow-hidden">
      ...
      <motion.div
        ...
        className="bg-card/90 backdrop-blur-2xl border-t border-border/40 rounded-t-[2rem] px-6 pt-7 pb-safe-bottom shadow-elevated"
        ...
        <form onSubmit={handleSubmit} className="space-y-4 max-w-md mx-auto">
          {!isLogin && (
            <div className="relative">
              <Input
                ...
                required={!isLogin}
                className="h-13 rounded-2xl text-sm bg-muted/30 border-border text-foreground placeholder:text-muted-foreground/50 focus:border-accent focus:ring-accent/20 pl-4"
              />
            </div>
          )}
          ...
          <div className="relative">
            ...
            <Input
              ...
              className="h-13 rounded-2xl text-sm bg-muted/30 border-border text-foreground placeholder:text-muted-foreground/50 focus:border-accent focus:ring-accent/20 pl-11"
              required
              maxLength={16}
            />
          </div>
          ...
          <div className="relative">
            ...
            <Input
              ...
              required
              minLength={6}
              className="h-13 rounded-2xl text-sm bg-muted/30 border-border text-foreground placeholder:text-muted-foreground/50 focus:border-accent focus:ring-accent/20 pl-11 pr-11"
            />
            <button
              type="button"
              onClick={() => setShowPassword(v => !v)}
              className="absolute right-3.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground/70 transition-colors"
              tabIndex={-1}
            >
          ...
          <label className="flex items-center gap-2.5 cursor-pointer select-none group">
            ...
            <span className={`text-xs transition-colors ${rememberMe ? 'text-foreground/80' : 'text-muted-foreground group-hover:text-foreground/60'}`}>
              Lembrar meu login
            </span>
          </label>
